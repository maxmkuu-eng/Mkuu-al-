...
public class SmsSenderPlugin extends Plugin {
    ...
    public void sendSms(PluginCall call) {
        if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.SEND_SMS) != PackageManager.PERMISSION_GRANTED) {
            call.reject("SEND_SMS_PERMISSION_REQUIRED");
            return;
        }
        ...
        try {
            ...
            for (int i = 0; i < parts.size(); i++) {
                ...
                sent.putExtra("to", to);
                sent.putExtra("part", i + 1);
                sent.putExtra("total", parts.size());
                Intent delivered = new Intent(DELIVERED_ACTION).setPackage(getContext().getPackageName());
                delivered.putExtra("to", to);
                delivered.putExtra("part", i + 1);
                delivered.putExtra("total", parts.size());
            ...
            JSObject ret = new JSObject();
            ret.put("status", "sending");
            ret.put("to", to);
            ret.put("parts", parts.size());
            ret.put("timestamp", System.currentTimeMillis());
            call.resolve(ret);
        } catch (Exception e) {
            call.reject("SMS_SEND_FAILED", e);
        }
    }
}
