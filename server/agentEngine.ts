...
function isCurrentGovernmentQuestion(message: string): boolean {
  const text = String(message || '').toLowerCase();
  return includesAny(text, CURRENT_GOVERNMENT_TERMS) || (/\b(sasa|wa sasa|kwa sasa|leo|current|latest)\b/.test(text) && /\b(waziri|rais|serikali|kiongozi|wizara|cabinet)\b/.test(text));
}
...
export class UniversalAgentEngine {
  ...
  public async execute(request: AgentRequest): Promise<AgentResult> {
    ...
    if (intent === 'image') {
      const result = await imageService.processImage({ userId: request.userId, prompt: request.message, attachments: request.attachments || [] });
      return { intent, reply: result.explanation, cleanSpeechText: result.explanation, generatedFiles: [result.file], memoriesExtracted: [], peopleRecognized: [], aiProvider: 'Google Gemini', chatModel: result.modelUsed, latencyMs: Date.now() - started };
    }
    const liveAwareMessage = isCurrentGovernmentQuestion(request.message)
      ? `[CURRENT_OFFICIAL_GOVERNMENT_INFORMATION — LIVE SEARCH REQUIRED]\n${request.message}`
      : request.message;
