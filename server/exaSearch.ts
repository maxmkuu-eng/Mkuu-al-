...
function extractOpponentAnswer(query:string,results:any[]){
  ...
  const aliases:Record<string,string>= {
    yanga:'young africans', 'yanga sc':'young africans', 'young africans':'young africans',
    simba:'simba', 'simba sc':'simba', azam:'azam', 'azam fc':'azam',
    'coastal union':'coastal union', 'coastal union fc':'coastal union'
  };
  ...
  for(const item of results){
    ...
    if(opp && opp.length<80){
      ...
      return `${day} ${team.replace(/\b\w/g,c=>c.toUpperCase())} ${future?'anacheza':'alicheza'} na ${opp}.`;
    }
  }
  return null;
}
