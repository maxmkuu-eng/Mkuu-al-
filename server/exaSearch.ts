...
// Current/live/news intent must not fall through to Gemini or a stale two-day cache.
...
function extractOpponentAnswer(query:string,results:any[]){
  ...
  for(const item of results){
    ...
    const m=h.match(a)||h.match(b); let opp=(m?.[1]||'').replace(/\s*(live score|live result|result|score|today|leo|tonight|scheduled|kick[- ]?off).*$/i,'').trim();
    if(!opp){const title=String(item?.title||'');const direct=title.match(/(?:Young Africans|Yanga(?: SC)?)\s+(?:vs\.?|v\.?|versus)\s+(.+)/i);if(direct)opp=direct[1].replace(/\s*(?:[-|:].*)$/,'').trim();}
    if(opp&&opp.length<80){const day=/\b(jana|yesterday)\b/i.test(q)?'Jana':/\b(juzi)\b/i.test(q)?'Juzi':/\b(kesho|tomorrow)\b/i.test(q)?'Kesho':'Leo';return `${day} ${team.replace(/\b\w/g,c=>c.toUpperCase())} ${future?'anacheza':'alicheza'} na ${opp}.`;}
  }
  return null;
}

export async function searchWithExa(query:string):Promise<ExaSearchResult>{
  const apiKey=resolveExaApiKey(), dates=tanzaniaDateContext(), fresh=isFreshOrRelativeQuery(query), social=isSocialQuery(query), sports=isSportsQuery(query), news=isNewsQuery(query), finalResult=isFinalResultQuery(query), opponent=isOpponentQuestion(query);
  const requestedDate=/\b(jana|yesterday)\b/i.test(query)?dates.yesterday:/\b(juzi)\b/i.test(query)?dates.twoDaysAgo:dates.today;
  const q=fresh?`${query}\nTANZANIA TIME: ${dates.formatted}. Current date=${dates.today}. Requested event date=${requestedDate}. ${sports?(finalResult?'FINAL SPORTS RESULT: find the completed match on the requested date, final/full-time score and opponent. Ignore previews and scheduled fixtures.':'SPORTS FIXTURE: find the match scheduled for the requested date, opponent, venue and kick-off. For questions asking who a team plays today/leo, prioritize an actual fixture scheduled on the requested date; do not answer from previous results or generic news.'):news?'CURRENT NEWS/EVENT: search recent credible reporting for the specific person/event. Do not answer “no official information” merely because the person did not post directly. If credible news reports or a family/representative statement confirm the event, state it as confirmed and give the publication/event date. Compare multiple recent sources when available.':'Use the requested event date and newest credible evidence.'}`:`${query}\nCURRENT TANZANIA TIME: ${dates.formatted}`;
  ...
  if(fresh){
    if(sports){body.startPublishedDate=finalResult?new Date(`${requestedDate}T00:00:00+03:00`).toISOString():new Date(`${dates.twoDaysAgo}T00:00:00+03:00`).toISOString();}
    else if(news){body.startPublishedDate=new Date(Date.now()-30*86400000).toISOString();}
    else {body.startPublishedDate=new Date(Date.now()-7*86400000).toISOString();}
    body.endPublishedDate=new Date(Date.now()+86400000).toISOString();body.maxAgeHours=0;
  }
  ...
  const data=await response.json() as any; const raw=Array.isArray(data?.results)?data.results:[]; const ranked=sports&&finalResult?[...raw].sort((a:any,b:any)=>resultStrength(b)-resultStrength(a)):raw; const results=ranked.slice(0,sports?10:(news?10:8)); const citations=results.filter((x:any)=>x?.url).map((x:any)=>({title:String(x.title||x.url).trim(),url:String(x.url).trim()}));
  let answer='';
  ...
  else answer=String(data?.output?.content||'').trim()||results.slice(0,social?3:(news?5:2)).map((x:any)=>String(x.highlights?.[0]||x.summary||x.text||'').trim().slice(0,1000)).filter(Boolean).join('\n\n').trim();
