...
export async function searchWithTavily(query:string):Promise<string>{
 ...
 // WORLDWIDE WEB: every current/factual search gets broad web + news evidence.
 ...
 // SOCIAL MEDIA: search major public social platforms explicitly when relevant, and also for public-figure/entertainment queries.
 if(social || !sports || /zuchu|diamond|wasanii|msanii|celebrity|artist|singer|actor|actress|baby|pregnan|amejifungua|ameoa|ameolewa|relationship|marriage|breakup|ujauzito|mtoto|kujifungua/i.test(query)){
