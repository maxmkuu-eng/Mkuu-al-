...
// Final normalization is intentionally structural and idempotent. Earlier build
// steps may have already rewritten this file, so never depend on one exact layout.
...
// Replace or inject health method before processChat.
...
// Remove the entire legacy SDK executor by method boundary, then install REST.
...
// No browser/SDK/Tavily runtime is allowed in GeminiService. Do not fail on harmless
// comments/strings; only reject executable legacy references that can break runtime.
s=s.replace(/\b(?:client|this\.aiClient)\.models\.generateContent\s*\(/g,'this.executeGeminiCallWithFallback({contents:params.contents,config:params.config,preferredModel:PERSONAL_CHAT_MODEL}) /* legacy call normalized */(');
