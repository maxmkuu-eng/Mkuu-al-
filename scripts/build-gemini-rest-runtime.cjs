...
// Idempotent normalization. This script must never fail because an earlier build
// step has already applied the same transformation.
...
const forbidden = [
  ...
  /\bsearchWithTavily\s*\(/,
  /\bprivate\s+getClient\s*\(\s*\)\s*:\s*GoogleGenAI\s*\{/ 
];
