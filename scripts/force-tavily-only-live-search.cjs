...
// ---------------------------------------------------------------------------
// BACKEND: Tavily is the only authority for live/current information.
// ---------------------------------------------------------------------------
patchFile('server/geminiService.ts', (source) => {
  if (!source.includes("import { searchWithTavily } from './tavilySearch.js';")) {
    throw new Error('MKUU: Tavily import marker not found.');
  }
  ...
  if (!source.includes('webSources: Array<{ title: string; url: string }>;')) {
    ...
    if (!source.includes(marker)) throw new Error('MKUU: ChatProcessResult marker not found.');
  ...
  if (searchStartIndex < 0 || fallbackStart < 0 || outerCatchEnd < 0) {
    throw new Error('MKUU: Tavily-only live-search markers not found; refusing unsafe rewrite.');
  }
  ...
  if (/buildConversationHistory\(\s*conversationHistory/i.test(liveBlock)) {
    throw new Error('MKUU: historical conversation remains exposed to live-search synthesis.');
  }
  return source;
});

// ---------------------------------------------------------------------------
// API: do not rewrite live questions into a Google Search instruction, and
// return Tavily sources to the client with the same response that was searched.
// ---------------------------------------------------------------------------
patchFile('server.ts', (source) => {
  ...
  if (blockStart >= 0 && resultCall > blockStart) {
    ...
    source = source.replace(
      'geminiService.processChat({userId:DEFAULT_USER_ID,message:searchMessage,conversationHistory:effectiveHistory,isVoice,attachments})',
      'geminiService.processChat({userId:DEFAULT_USER_ID,message,conversationHistory:effectiveHistory,isVoice,attachments})',
    );
  }
  ...
  if (source.includes(oldReturn) && !source.includes('webSources:result.webSources')) {
    source = source.replace(oldReturn, 'generatedFiles:result.generatedFiles,webSources:result.webSources||[],aiProvider:result.aiProvider,chatModel:result.chatModel,latencyMs:result.latencyMs', 1);
  }
  if (/Tafuta Google na uthibitishe taarifa za sasa/i.test(source)) {
    throw new Error('MKUU: server API still injects Google Search instructions.');
  }
  return source;
});

// ---------------------------------------------------------------------------
// CLIENT: live/current questions must never use a stored Gemini API key or
...
patchFile('src/services/aiEngine.ts', (source) => {
  ...
  if (!source.includes('webSources?: Array<{ title: string; url: string }>;')) {
    const marker = "  generatedFiles?: GeneratedFileSummary[];\n  engineUsed:";
    if (!source.includes(marker)) throw new Error('MKUU: ChatEngineResult marker not found.');
    source = source.replace(marker, "  generatedFiles?: GeneratedFileSummary[];\n  webSources?: Array<{ title: string; url: string }>;\n  engineUsed:", 1);
  }
  ...
  if (source.includes(nativeMarker) && !source.includes("webSources: serverRes.webSources || []")) {
    source = source.replace(nativeMarker, "    intent: serverRes.intent || 'chat',\n    webSources: serverRes.webSources || [],\n  };", 1);
  }

  const streamDoneMarker = "  let reply = '';\n  emitStream('', false);";
  if (source.includes(streamDoneMarker) && !source.includes('let donePayload: any = null;')) {
    source = source.replace(streamDoneMarker, "  let reply = '';\n  let donePayload: any = null;\n  emitStream('', false);", 1);
  }
  ...
  if (source.includes(donePayloadLine) && !source.includes('donePayload = payload;')) {
    source = source.replace(donePayloadLine, "        if (payload.type === 'delta' && payload.text) { reply += payload.text; emitStream(payload.text, false); }\n        if (payload.type === 'done') donePayload = payload;\n        if (payload.type === 'error') throw new Error(payload.message || 'Streaming error');", 1);
  }
  ...
  if (source.includes(streamReturn)) {
...
console.log('MKUU: Tavily is authoritative for worldwide live web/news/social search; live requests are forced through backend/Tavily, Gemini only synthesizes Tavily evidence, and Tavily sources are returned to the UI.');