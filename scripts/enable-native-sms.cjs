...
patch(
  'src/components/PeopleCenter.tsx',
  'mkuu-send-sms-modal',
...
// Add imports/state/handlers/button using stable source anchors.
let p = 'src/components/PeopleCenter.tsx';
let s = fs.readFileSync(p, 'utf8');
if (!s.includes("../services/smsService")) {
  s = s.replace("import { Person } from '../types';", "import { Person } from '../types';\nimport { getSimCards, sendNativeSms, SmsSender } from '../services/smsService';");
}
if (!s.includes('smsModalPerson')) {
  s = s.replace(
  ...
  if (!s.includes('id={`send-sms-${person.id}`}')) {
    s = s.replace(
  ...
  console.log('[SMS] PeopleCenter patched');
}
