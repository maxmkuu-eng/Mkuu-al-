...
// This patch is intentionally idempotent because the build chain invokes it twice.
{
  ...
  if (!s.includes("import React, { useState, useEffect, useRef } from 'react';")) {
  ...
  if (!s.includes('if (mkuuAbortControllerRef.current === abortController)')) {
    s = s.replace(
      "    } finally {\n      setIsLoading(false);\n    }\n  };",
...
// AI engine: propagate AbortSignal into network generation routes.
{
  ...
  if (!s.includes("    signal: params.signal,\n    body: JSON.stringify({\n      prompt: params.message,")) {
    s = s.replace(
      "    method: 'POST',\n    body: JSON.stringify({\n      prompt: params.message,",
      "    method: 'POST',\n    signal: params.signal,\n    body: JSON.stringify({\n      prompt: params.message,"
    );
  }
  if (!s.includes('body: JSON.stringify(body), signal: params.signal')) {
    s = s.replace(
  ...
  if (!s.includes('Accept: \'text/event-stream\', \'Content-Type\': \'application/json\', \'Cache-Control\': \'no-cache\' }, signal: params.signal')) {
    s = s.replace(
...
console.log('MKUU: functional Stop generation + working speaker playback enabled (idempotent).');
