...
function patch(file, replacements) {
  ...
  for (const [from, to] of replacements) {
    ...
    if (!source.includes(from)) {
      // ChatView/Navigation may have been intentionally redesigned. Do not fail
      // the production build because an old branding patch target disappeared.
      console.log(`[BRANDING] Target changed in ${path.relative(root, file)}; skipping this legacy patch.`);
      continue;
    }
...
console.log('[BRANDING] MKUU AI branding patch completed without blocking redesigned UI files.');
