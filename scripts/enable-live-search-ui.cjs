...
function patch(filePath, replacements) {
  ...
  for (const [from, to] of replacements) {
    ...
    if (!source.includes(from)) {
      throw new Error(`MKUU patch target not found in ${filePath}: ${from.slice(0, 160)}`);
    }
...
patch('server.ts', [
  ...
  [
    ...
    "    const result=await geminiService.processChat({userId:DEFAULT_USER_ID,message:searchMessage,conversationHistory:effectiveHistory,isVoice,attachments});\n    const webSources=currentFactQuery?getLastTavilySources():[];",
  ],
  [
  ...
  [
    "    try{const result=await processChatRequest(req);res.write(`data: ${JSON.stringify({type:'delta',text:result.reply})}\\n\\n`);res.write(`data: ${JSON.stringify({type:'done',...result})}\\n\\n`);res.end();}",
    "    try{const result=await processChatRequest(req);res.write(`data: ${JSON.stringify({type:'delta',text:result.reply})}\\n\\n`);res.write(`data: ${JSON.stringify({type:'done',...result})}\\n\\n`);res.end();}",
  ],
]);
...
patch('src/components/ChatView.tsx', [
  [
    ...
    "                      {msg.webSources && msg.webSources.length > 0 && (\n                        <div className=\"mt-3 pt-2.5 border-t border-[#222222] w-full flex flex-col items-end not-italic font-sans\">\n                          <div className=\"text-[10px] uppercase tracking-wider text-[#777777] mb-1.5\">Vyanzo vya taarifa</div>\n                          <div className=\"flex flex-wrap justify-end gap-1.5 max-w-full\">\n                            {msg.webSources.map((source, idx) => (\n                              <a key={idx} href={source.url} target=\"_blank\" rel=\"noopener noreferrer\" className=\"inline-flex max-w-[260px] items-center rounded-lg border border-[#D4AF37]/25 bg-[#D4AF37]/5 px-2 py-1 text-[10px] font-semibold text-[#D4AF37] hover:bg-[#D4AF37]/15 hover:text-[#F5F2ED] transition\" title={source.url}>\n                                <span className=\"truncate\">{source.title || source.url}</span>\n                              </a>\n                            ))}\n                          </div>\n                        </div>\n                      )}\n\n                      {/* Footer Controls for AI Message */}",
  ],
]);

console.log('MKUU: live-search sources are passed through chat and rendered as clickable links.');
