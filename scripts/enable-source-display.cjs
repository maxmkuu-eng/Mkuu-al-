...
patch('src/components/ChatView.tsx', 'visible per-response source footer', (source) => {
  ...
  // Insert only at a completed assistant-message boundary. This avoids placing JSX
  // inside the nested generated-file/person-recognition expressions.
  const needle = '</div><div className="flex items-center space-x-3 px-2 text-[10px] text-[#888888]"><span>MKUU AI •';
  const block = `{msg.webSources && msg.webSources.length > 0 && (\n  <div className="mt-3 pt-3 border-t border-[#222222] not-italic font-sans">\n    <div className="flex items-center justify-end mb-2 text-[10px] font-bold uppercase tracking-wider text-[#D4AF37]">\n      <span>Vyanzo vya Taarifa</span>\n    </div>\n    <div className="flex flex-wrap justify-end gap-1.5">\n      {msg.webSources.map((source, idx) => (\n        <a key={source.url || idx} href={source.url} target="_blank" rel="noopener noreferrer" className="inline-flex max-w-full items-center px-2.5 py-1.5 rounded-lg bg-white/5 border border-[#333333] text-[10px] text-[#BBBBBB] hover:text-[#D4AF37] hover:border-[#D4AF37]/50 transition" title={source.url}>\n          <span className="truncate max-w-[220px]">{source.title || source.url}</span>\n        </a>\n      ))}\n    </div>\n  </div>\n)}\n`;
  if (!source.includes(needle)) throw new Error('MKUU: ChatView source insertion boundary not found.');
  return source.replace(needle, `</div>${block}<div className="flex items-center space-x-3 px-2 text-[10px] text-[#888888]"><span>MKUU AI •`);
});
