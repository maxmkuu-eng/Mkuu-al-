...
const replaceOnce = (s, from, to) => s.includes(from) ? s.replace(from, to) : s;

// 1) Make Exa live-search accuracy deterministic for current officials and sports results.
{
  ...
  s = s.replace(
    ...
    "if(fresh){if(sports)body.startPublishedDate=finalResult?new Date(`${requestedDate}T00:00:00+03:00`).toISOString():new Date(`${dates.twoDaysAgo}T00:00:00+03:00`).toISOString();else if(news)body.startPublishedDate=new Date(Date.now()-60*86400000).toISOString();else body.startPublishedDate=new Date(Date.now()-30*86400000).toISOString();body.endPublishedDate=new Date(Date.now()+86400000).toISOString();body.maxAgeHours=0;}"
  );

  write(file, s);
}

// 2) Never let the browser's stored Gemini key bypass Exa for live/current questions.
{
...
// 3) Expose the real health state and source links from the backend API.
{
...
// 5) Make ChatProcessResult carry source links from the Exa route.
{
